...
ALL_TARGETS:=lib/libAria.$(sosuffix) utils examples tests docs params lib/libArNetworking.$(sosuffix) swig arnetworking_docs arnetworking_swig lib/libArNetworking.a $(STATIC_TARGETS)

DIST_TARGETS:=lib/libAria.$(sosuffix) params docs CommandLineOptions.txt.in \
  lib/libArNetworking.$(sosuffix) ArNetworking/CommandLineOptions.txt.in examples/demo \
  examples/demoStatic examples/seekurPower examples/mtxPower ArNetworking/examples/serverDemo \
  ArNetworking/examples/serverDemoStatic ArNetworking/examples/clientDemo \
  ArNetworking/examples/clientDemoStatic   \
  arnetworking_docs

ifndef NO_DIST_WRAPPERS
DIST_TARGETS:=$(DIST_TARGETS) java python arnetworking_wrappers
endif
...
arnetworking_java:
	$(MAKE) -C ArNetworking java

arnetworking_python:
	$(MAKE) -C ArNetworking python

arnetworking_docs:
	$(MAKE) -C ArNetworking docs

arnetworking_all:
	$(MAKE) -C ArNetworking all
...
distclean: clean cleanUtils cleanExamples cleanAdvanced cleanModules cleanDep
	$(MAKE) -C ArNetworking clean cleanDep
...
lib/libArNetworkingJava.$(sosuffix): FORCE
	$(MAKE) -C ArNetworking ../$@

python/ArNetworkingPy.py: FORCE
	$(MAKE) -C ArNetworking ../$@

python/_ArNetworkingPy.$(sosuffix): FORCE
	$(MAKE) -C ArNetworking ../$@
...
advanced/%.$(sosuffix): advanced/%.$(CFILEEXT) lib/libAria.$(sosuffix)
	$(CXX) $(CXXFLAGS) $(CXXINC) -shared $< -o $@

advanced/%$(binsuffix): advanced/%.$(CFILEEXT) lib/libAria.$(sosuffix) 
	$(CXX) $(CXXFLAGS) $(CXXINC) $< -o $@ $(CXXLINK)
...
lib/libArNetworking.$(sosuffix): FORCE
	$(MAKE) -C ArNetworking ../$@

lib/libArNetworking.a: FORCE 
	$(MAKE) -C ArNetworking ../$@

ArNetworking/examples/%: ArNetworking/examples/%.cpp lib/libArNetworking.$(sosuffix)
...
install: all arnetworking_all
	...
	find    include src tests utils params docs examples advanced maps \
	        ArNetworking java javaExamples python pythonExamples matlab \
	...
	$(INSTALL) -D --strip -m 666 lib/libAria.$(sosuffix) lib/libArNetworking.$(sosuffix) $(DESTDIR)$(INSTALL_DIR)/lib/
	...
	if test -f lib/libArNetworkingJava.$(sosuffix); then $(INSTALL) --strip -m 666 lib/libArNetworkingJava.$(sosuffix) $(DESTDIR)$(INSTALL_DIR)/lib/; else echo Warning: lib/libArNetworkingJava.$(sosuffix) not found. Enter the ArNetworking directory and run \"make java\" to build.; fi
